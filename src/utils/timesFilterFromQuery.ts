...
function baseFilter({ query, defaults }: BaseArgs): Omit<TimesFilter, 'scope'> {
  return {
    ...
    invalidated: query.invalidated !== undefined ? query.invalidated === 'true' : undefined,
  };
}
...
const forMap = function(
  ...
  return { ...baseFilter({ query, defaults }), scope: { kind: 'map', map: mapName } };
};
...
const forPlayer = function(
  ...
  return { ...baseFilter({ query, defaults }), scope: { kind: 'player', steamid, wr } };
};
