...
function isAllZero(slice: ArrayBufferSlice): boolean {
    const wordCount = slice.byteLength >>> 2;
    if (wordCount > 0) {
        const u32 = slice.createTypedArray(Uint32Array, 0, wordCount);
        for (let i = 0; i < u32.length; i++)
            if (u32[i] !== 0) return false;
    }
    const tail = slice.byteLength & 3;
    if (tail > 0) {
        const u8 = slice.createTypedArray(Uint8Array, wordCount * 4, tail);
        for (let i = 0; i < u8.length; i++)
            if (u8[i] !== 0) return false;
    }
    return true;
}
...
export class BSPFile {
    ...
    constructor(buffer: ArrayBufferSlice, public mapname: string, private variant: BSPFileVariant = BSPFileVariant.Default) {
        ...
        // An "empty or all-zero" lighting lump means lighting wasn't baked.
        // bhop_egoist ships LIGHTING_HDR with size 0 and a 3.7 MB LIGHTING
        // lump that's 100% zero — upstream's byteLength check picks LDR
        // and every face decodes to black. We surface a flag so the loader
        // leaves samples=null, which the runtime turns into a white
        // full base-texture brightness instead of pitch black).
        let lighting: ArrayBufferSlice | null = null;
        let hasValidLighting = true;
        ...
        if (preferHDR) {
            ...
            if (lighting === null || lighting.byteLength === 0 || isAllZero(lighting)) {
        ...
        for (let i = 0, idx = 0x00; idx < facelist.byteLength; i++, idx += 0x38, numfaces++) {
            ...
            if (hasValidLighting && lightofs !== -1)
