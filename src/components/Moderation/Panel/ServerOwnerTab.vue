...
const loadServers = async () => {
  ...
  try {
    ...
    if (!selectedServerId.value && ownedServers.value.length > 0) {
      selectedServerId.value = ownedServers.value[0]._id
    }
  } catch (e) {
...
watch(ownedServers, () => {
  if (selectedServerId.value && !ownedServers.value.some(s => s._id === selectedServerId.value)) {
    selectedServerId.value = ownedServers.value[0]?._id ?? ''
    ...
    <div class="bg-main-800 border border-main-500 rounded-lg p-4 space-y-3">
      ...
      <div v-else-if="ownedServers.length > 0" class="grid sm:grid-cols-[1fr_auto] gap-3 items-end">
        <div>
          ...
          >
            <option v-for="s in ownedServers" :key="s._id" :value="s._id">
